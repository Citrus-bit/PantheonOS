#!/usr/bin/env python
"""
Compare original NumPy Harmony vs evolved NumPy Harmony on TMA data.
Uses numpy-only implementations to avoid PyTorch DLL issues.
"""
...
def run_comparison():
    ...
    # Load numpy-based harmony implementations
    print("\nLoading NumPy Harmony implementations...")

    # Original numpy harmony (from results_pbmc - before evolution)
    harmony_original = load_module("harmony_original", example_dir / "results_pbmc" / "harmony_best.py")

    # Evolved numpy harmony (best from previous evolution)
    harmony_evolved = load_module("harmony_evolved", example_dir / "results_pbmc" / "harmony_52.py")

    # Run original harmony
    print("\nRunning original NumPy Harmony...")
    ...
    print(f"  Iterations: {len(hm_original.objectives)}")

    # Run evolved harmony
    print("\nRunning evolved NumPy Harmony...")
    start_time = time.time()
    ...
    # Try to compute UMAP (optional - may fail if umap not installed properly)
    try:
        ...
        print("  Original Harmony corrected...")
    ...
    if has_umap:
        ...
        datasets = [
            (umap_original, "Uncorrected", metrics["Uncorrected"]),
            (umap_harmony, "Original Harmony", metrics["Original Harmony"]),
        ...
        plt.suptitle("TMA Data: NumPy Harmony Evolution Comparison", fontsize=14, fontweight='bold')
        plt.tight_layout()
        fig1.savefig(output_dir / "tma_umap_comparison_numpy.png", dpi=150, bbox_inches='tight')
        print(f"  Saved: results_official/tma_umap_comparison_numpy.png")

    # Create figure 2: Performance comparison bar chart
    print("\nCreating performance comparison figure...")
    fig2, axes2 = plt.subplots(1, 3, figsize=(14, 4))

    methods = ["Uncorrected", "Original\nHarmony", "Evolved\nHarmony"]
    colors = ["#8b949e", "#58a6ff", "#238636"]

    # Mixing score
    ax = axes2[0]
    mixing_scores = [metrics["Uncorrected"]["mixing_score"],
                     metrics["Original Harmony"]["mixing_score"],
    ...
    ax = axes2[1]
    bio_scores = [metrics["Uncorrected"]["bio_score"],
                  metrics["Original Harmony"]["bio_score"],
    ...
    # Execution time
    ax = axes2[2]
    time_methods = ["Original\nHarmony", "Evolved\nHarmony"]
    times = [metrics["Original Harmony"]["time"], metrics["Evolved Harmony"]["time"]]
    ...
    speedup = time_original / time_evolved if time_evolved > 0 else 1
    ...
    plt.suptitle("TMA Data: NumPy Harmony Performance Comparison", fontsize=14, fontweight='bold')
    plt.tight_layout()
    fig2.savefig(output_dir / "tma_performance_comparison_numpy.png", dpi=150, bbox_inches='tight')
    print(f"  Saved: results_official/tma_performance_comparison_numpy.png")

    # Create figure 3: Combined score comparison
    print("\nCreating combined score figure...")
    fig3, ax3 = plt.subplots(figsize=(10, 6))

    combined_uncorrected = 0.45 * metrics["Uncorrected"]["mixing_score"] + 0.45 * metrics["Uncorrected"]["bio_score"]
    combined_original = 0.45 * metrics["Original Harmony"]["mixing_score"] + 0.45 * metrics["Original Harmony"]["bio_score"]
    combined_evolved = 0.45 * metrics["Evolved Harmony"]["mixing_score"] + 0.45 * metrics["Evolved Harmony"]["bio_score"]

    categories = ["Mixing\nScore", "Bio\nConservation", "Combined\nScore"]
    uncorrected_vals = [metrics["Uncorrected"]["mixing_score"], metrics["Uncorrected"]["bio_score"], combined_uncorrected]
    original_vals = [metrics["Original Harmony"]["mixing_score"], metrics["Original Harmony"]["bio_score"], combined_original]
    evolved_vals = [metrics["Evolved Harmony"]["mixing_score"], metrics["Evolved Harmony"]["bio_score"], combined_evolved]

    x = np.arange(len(categories))
    width = 0.25

    bars1 = ax3.bar(x - width, uncorrected_vals, width, label='Uncorrected', color='#8b949e', edgecolor='white')
    bars2 = ax3.bar(x, original_vals, width, label='Original Harmony', color='#58a6ff', edgecolor='white')
    bars3 = ax3.bar(x + width, evolved_vals, width, label='Evolved Harmony', color='#238636', edgecolor='white')

    ax3.set_ylabel('Score')
    ax3.set_title('TMA Data: Comprehensive Performance Comparison\n(NumPy Harmony Versions)', fontsize=14, fontweight='bold')
    ax3.set_xticks(x)
    ...
    plt.tight_layout()
    fig3.savefig(output_dir / "tma_combined_comparison_numpy.png", dpi=150, bbox_inches='tight')
    print(f"  Saved: results_official/tma_combined_comparison_numpy.png")

    # Create figure 4: Improvement summary
    print("\nCreating improvement summary figure...")
    fig4, ax4 = plt.subplots(figsize=(8, 5))

    improvement_metrics = ["Mixing Score", "Bio Conservation", "Combined Score", "Speed"]
    original_scores = [metrics["Original Harmony"]["mixing_score"],
                       metrics["Original Harmony"]["bio_score"],
                       combined_original,
                       time_original]
    ...
    # For speed, improvement is when evolved is faster (lower time)
    improvements = []
    for i, (e, o) in enumerate(zip(evolved_scores, original_scores)):
        if i == 3:  # Speed (time)
            improvements.append((o - e) / o * 100)  # Positive if faster
        else:
            improvements.append((e - o) / o * 100)
    ...
    ax4.set_title('Evolution Improvement: Original → Evolved Harmony (NumPy)', fontsize=14, fontweight='bold')
    ...
    ax4.set_xlim(min(min(improvements) - 5, -10), max(max(improvements) + 5, 10))
    plt.tight_layout()
    fig4.savefig(output_dir / "tma_improvement_summary_numpy.png", dpi=150, bbox_inches='tight')
    print(f"  Saved: results_official/tma_improvement_summary_numpy.png")

    print(f"\nDone! Generated figures in results_official/")

    # Print summary
    print("\n" + "=" * 60)
    print("SUMMARY (NumPy Harmony Evolution)")
    print("=" * 60)
    print(f"\nMixing Score:")
    print(f"  Original: {metrics['Original Harmony']['mixing_score']:.4f}")
    print(f"  Evolved:  {metrics['Evolved Harmony']['mixing_score']:.4f}")
    print(f"  Change:   {(metrics['Evolved Harmony']['mixing_score'] - metrics['Original Harmony']['mixing_score']) / metrics['Original Harmony']['mixing_score'] * 100:+.2f}%")

    print(f"\nBio Conservation:")
    print(f"  Original: {metrics['Original Harmony']['bio_score']:.4f}")
    print(f"  Evolved:  {metrics['Evolved Harmony']['bio_score']:.4f}")
    print(f"  Change:   {(metrics['Evolved Harmony']['bio_score'] - metrics['Original Harmony']['bio_score']) / metrics['Original Harmony']['bio_score'] * 100:+.2f}%")

    print(f"\nCombined Score:")
    print(f"  Original: {combined_original:.4f}")
    print(f"  Evolved:  {combined_evolved:.4f}")
    print(f"  Change:   {(combined_evolved - combined_original) / combined_original * 100:+.2f}%")
